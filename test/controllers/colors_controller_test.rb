...
class ColorsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create color" do
    assert_difference('Color.count') do
      post colors_url, params: { color: { categories_id: @color.categories_id, color: @color.color } }
    end
  ...
  test "should update color" do
    patch color_url(@color), params: { color: { categories_id: @color.categories_id, color: @color.color } }
